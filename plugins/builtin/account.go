...
// Ported from PagerMaid's account.py: username/name/bio/pfp/rmpfp.
type AccountPlugin struct{}

func NewAccount() *AccountPlugin { return &AccountPlugin{} }
...
func (p *AccountPlugin) handleUsername(ctx *interfaces.CommandContext) error {
	newName := ""
	if ctx.ArgCount() > 0 {
...
func (p *AccountPlugin) handleName(ctx *interfaces.CommandContext) error {
	if ctx.ArgCount() == 0 {
...
func (p *AccountPlugin) handleBio(ctx *interfaces.CommandContext) error {
	bio := ctx.RawArgs
...
func (p *AccountPlugin) handleRmPfp(ctx *interfaces.CommandContext) error {
	limit := 1
	if ctx.ArgCount() > 0 {
		if ctx.GetArg(0) == "all" {
			limit = 0
