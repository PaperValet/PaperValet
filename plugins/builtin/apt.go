...
// AptPlugin is the plugin package manager (renamed from ppm).
// Pure plugin manager: install/remove/load/unload/list/info/search.
...
func (p *AptPlugin) Description() string { return "📦 插件包管理器（动态插拔）" }

func (p *AptPlugin) Init(_ context.Context, mgr plugin.Manager) error {
	p.mgr = mgr
	return mgr.RegisterCommand(&interfaces.Command{
		Name:        "apt",
		Aliases:     []string{"ppm", "plugin", "plugins", "pkg"},
		Description: "插件包管理器 — 安装/卸载/加载/管理插件",
		Usage:       "apt <子命令> [参数...]",
		Plugin:      p.Name(),
		Category:    "core",
		OwnerOnly:   true,
...
func (p *AptPlugin) handleApt(ctx *interfaces.CommandContext) error {
	...
	case "info", "status":
		if len(subArgs) == 0 {
			return ctx.Edit("用法: apt info <插件名>")
		}
	...
	case "install", "add", "get":
		if len(subArgs) == 0 {
			return ctx.Edit("用法: apt install <插件名> [插件名...]")
		}
		return p.installPlugins(ctx, subArgs)
	case "remove", "rm", "delete", "uninstall":
		if len(subArgs) == 0 {
			return ctx.Edit("用法: apt remove <插件名> [插件名...]")
		}
		return p.removePlugins(ctx, subArgs)
	case "load", "enable":
		if len(subArgs) == 0 {
			return ctx.Edit("用法: apt load <插件名> [插件名...]")
		}
		return p.loadPlugins(ctx, subArgs)
	case "unload", "disable":
		if len(subArgs) == 0 {
			return ctx.Edit("用法: apt unload <插件名> [插件名...]")
		}
...
func (p *AptPlugin) showHelp(ctx *interfaces.CommandContext) error {
	return ctx.Edit(p.helpText())
}

func (p *AptPlugin) helpText() string {
	return `📦 <b>APT — 插件包管理器</b>

<b>管理命令:</b>
• <code>apt list</code> — 列出已安装的插件
• <code>apt loaded</code> — 列出已加载的插件
• <code>apt info &lt;name&gt;</code> — 查看插件详情

<b>安装与移除:</b>
• <code>apt install &lt;name&gt;</code> — 下载并安装插件
• <code>apt remove &lt;name&gt;</code> — 删除已安装的插件

<b>生命周期:</b>
• <code>apt load &lt;name&gt;</code> — 加载插件（启动）
• <code>apt unload &lt;name&gt;</code> — 卸载插件（停止）

<b>重载:</b>
• <code>reload &lt;name&gt;</code> — 热重载插件（独立命令）`
}
...
func (p *AptPlugin) listInstalled(ctx *interfaces.CommandContext) error {
	...
	if len(installed) == 0 && len(loaded) == 0 {
		return ctx.Edit("📦 暂无已安装的插件\n\n使用 <code>apt install &lt;name&gt;</code> 安装插件")
	}
...
func (p *AptPlugin) pluginInfo(ctx *interfaces.CommandContext, name string) error {
	...
	for _, n := range installed {
		if n == name {
			return ctx.Edit(fmt.Sprintf("💾 <b>%s</b> ⚪ 已安装但未加载\n\n使用 <code>apt load %s</code> 加载", name, name))
		}
	}

	return ctx.Edit(fmt.Sprintf("❌ 未找到插件: %s\n\n使用 <code>apt list</code> 查看已安装的插件", name))
}

func (p *AptPlugin) searchRegistry(ctx *interfaces.CommandContext, args []string) error {
	...
	if query != "" {
		...
		b.WriteString("使用 <code>apt install <name></code> 安装\n\n")
	}
...
func (p *AptPlugin) installPlugins(ctx *interfaces.CommandContext, names []string) error {
...
func (p *AptPlugin) removePlugins(ctx *interfaces.CommandContext, names []string) error {
...
func (p *AptPlugin) loadPlugins(ctx *interfaces.CommandContext, names []string) error {
...
func (p *AptPlugin) unloadPlugins(ctx *interfaces.CommandContext, names []string) error {
